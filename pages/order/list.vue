<template>
	<view>
		...
		<!--tab列表-->
		<view class="bg-white app-nav-tab-view">
		...
		<block v-for="(item,index) in order_list" :key="index" v-if="tab_cur == 0">
			<view class="bg-white app-order-tab-view" @tap="detailsTap">
				<!--店铺名称-->
				...
				<!--按钮-->
				<view class="app-btn-view" v-if="item.type == 1">
				...
				<!--按钮-->
				<view class="app-btn-view" v-if="item.type == 0">
					<button class="cu-btn line-black sm radius">联系卖家</button>
				</view>
			</view>
		</block>
		...
		<!--无数据-->
		<view class="bg-white app-null-view" v-if="tab_cur != 0">
			<view class="img-view">
				<view class="cu-avatar lg round" style="background-image:url('/static/app/img/aa6.png')"/>
			</view>
		...
		<!--到底了-->
		<view class="app-foot-tip-view" v-if="tab_cur == 0">
			<view class="img-view">
				<view class="cu-avatar round" style="background-image:url('/static/app/img/acn.png')"/>
			</view>
...
<style lang="scss">
	...
	/* #endif */
	@import "../../static/app/style/order-list.scss";
</style>
