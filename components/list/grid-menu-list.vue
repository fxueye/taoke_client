<template>
	<view class="app-scroll-view-box">
		<scroll-view class="app-scroll-view" scroll-x @scroll="navScroll">
			<view id="scroll1" class="app-scroll-view-item">
			...
			<view id="scroll2" class="app-scroll-view-item" v-if="ListData.length > 10">
				<view class="cu-list grid col-5 no-border">
					<block v-for="(item,index) in ListData" :key="index" v-if="index >= 10">
						<view class="cu-item" @tap="listTap(item,index)">
							<view class="grid-icon">
		...
		<view class="app-scroll-h-view" :style="{width: `${scrollPage * 16}px`}">
			<view class="box">
...
<script>
	...
	export default {
		...
		methods: {
			getScrollPageNum() {
				this.scrollPage =  utils.getPageNum(this.ListData.length,10);
			},
			navScroll(e) {
				//计算公式: 移动距离 /（A总 / B总）
