<template>
	<view>
		<block v-for="(item,index) in list_data" :key="index" v-if="getMinToMax(index)">
			<view class="bg-white margin-top padding radius app-trends">
				<!--用户信息-->
				...
				<!--内容-->
				<view class="margin-tb text-black app-text-content" @tap="listTap('contentTap',item,index)">
				...
				<!-- #ifndef APP-PLUS -->
				<view class="app-video-box" v-if="item.video">
				...
				<view class="app-img-grid-col" v-if="item.img.length > 0">
					<!--单图-->
				...
				<!--操作-->
				<view class="flex p-xs app-footer-tool">
